    ...
    IEnumerator FindPath(Vector3 startPos, Vector3 targetPos)
    {
        ...
        if (startNode.walkable)
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                /*
                 * Maybe make it so that after finding the nearest walkable node,
                 * itll just full send to the player until the player 
                 */
                if (!targetNode.walkable)
                {
                    Node nearestWalkableNode = FindNearestWalkableNode(targetNode);
                    if (nearestWalkableNode != null)
                    {
                        targetNode = nearestWalkableNode;
                        openSet.Add(startNode);
                    }
                    else
                        pathSuccess = false;
    ...
    public Node FindNearestWalkableNode(Node targetNode)
    {
        ...
        while (openSet.Count > 0)
        {
            ...
            foreach (Node neighbor in astarGrid.GetNeighbours(currentNode))
            {
                ...
                int newCost = GetDistance(targetNode, neighbor);
